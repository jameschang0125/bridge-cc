...
\NewDocumentCommand{\subbid}{m o}{%
  ...
  \else
    \def\bidLeftCell{}%
  \fi
  \bidLeftCell & {\ttfamily {\color{subbidTextColor}#1}\IfValueT{#2}{{\texttt{\color{subbidTextColor}:}\ \texttt{\color{subbidExtraColor}#2}}}}\tabularnewline
}
% Command to output standalone bids (bids without subbids)
\NewDocumentCommand{\flushbid}{}{%
  \ifnum\value{bidNeed}=1\relax
    \ifx\bidDesc\empty
      {\color{bidLabelColor}\texttt{\bfseries \bidLabel}} & \tabularnewline
    \else
      {\color{bidLabelColor}\texttt{\bfseries \bidLabel :}}{\texttt{\color{bidDescColor} \bidDesc}} & \tabularnewline
    \fi
    \setcounter{bidNeed}{0}%
  \fi
}
...
\else
  \def\bidLeftCell{}%
\fi
\bidLeftCell & {\ttfamily{\color{subbidTextColor}\phantom{oooo}#1}\IfValueT{#2}{{\texttt{\color{subbidTextColor}:}\ \texttt{\color{subbidExtraColor} #2}}}}\tabularnewline
}
\newenvironment{bidsemi}{%
  ...
  \ifnum\value{bidNeed}=1\relax
    \ifx\bidDesc\empty
      {\color{bidLabelColor}\texttt{\bfseries \bidLabel}} & \\
    \else
      {\color{bidLabelColor}\texttt{\bfseries \bidLabel :}}{\texttt{\color{bidDescColor} \bidDesc}} & \\
    \fi
  \fi
