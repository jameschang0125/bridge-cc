...
\NewDocumentCommand{\subbid}{m o}{%
  ...
  {\cellcolor{NeutralBG}\bidLeftCell} & {\cellcolor{NeutralBG}\ttfamily {\color{subbidTextColor}#1}\IfValueT{#2}{{\texttt{\color{subbidTextColor}:}\ \texttt{\color{subbidExtraColor}#2}}}}\tabularnewline
}
...
\AddShadeForCurrentRow
{\cellcolor{NeutralBG}\bidLeftCell} & {\cellcolor{NeutralBG}\ttfamily{\color{subbidTextColor}\phantom{oooo}#1}\IfValueT{#2}{{\texttt{\color{subbidTextColor}:}\ \texttt{\color{subbidExtraColor} #2}}}}\tabularnewline
}
\newenvironment{bidsemi}{%
  \begingroup
  \par\ttfamily\parskip=0pt\parindent=0pt%
  \setcounter{bidNeed}{0}\gdef\bidLabel{}\gdef\bidDesc{}%
  \setcounter{rowIndex}{0}\gdef\shadeCommands{}%
  ...
  \fi
  \CodeAfter
    % Define drawer now (after table is laid out) and then execute accumulated calls
    \renewcommand{\ShRight}[2]{%
      \begin{tikzpicture}[overlay, remember picture]
        \path[shade, left color=NeutralBG, right color=##2]
          (##1-2.north west) rectangle (##1-2.south east);
      \end{tikzpicture}%
    }%
    \shadeCommands
